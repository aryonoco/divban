...
export const configureSubordinateIds = (
	...
	withLock(
		...
		Effect.gen(function* () {
			...
			yield* appendSubidEntry(
				SYSTEM_PATHS.subuid as AbsolutePath,
				username,
				entry,
			);
			yield* appendSubidEntry(
				SYSTEM_PATHS.subgid as AbsolutePath,
				username,
				entry,
			);
		}),
...
export const createServiceUser = (
	...
	Effect.gen(function* () {
		...
		const retrySchedule = pipe(
			systemRetrySchedule,
			Schedule.whileInput((err: SystemError | GeneralError) =>
				isUidConflictError(err),
			),
		);
			...
			Effect.gen(function* () {
				...
				const uid = yield* Effect.acquireRelease(
					withLock(
						...
						Effect.gen(function* () {
							const allocatedUid =
								yield* allocateUidInternal(settings);
							yield* createUserWithUid(
								username,
								homeDir,
								shell,
								serviceName,
