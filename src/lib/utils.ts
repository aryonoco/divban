...
): Promise<string> => Bun.readableStreamToText(stream);

export const streamToBytes = async (
	stream: ReadableStream<Uint8Array>,
): Promise<Uint8Array> => await Bun.readableStreamToBytes(stream);

export const streamToJSON = <T>(
	stream: ReadableStream<Uint8Array>,
): Promise<T> => Bun.readableStreamToJSON(stream) as Promise<T>;

export const streamToArray = async <T>(
	stream: ReadableStream<T>,
): Promise<T[]> => await Bun.readableStreamToArray(stream);

export const streamToBlob = (
	stream: ReadableStream<Uint8Array>,
): Promise<Blob> => Bun.readableStreamToBlob(stream);
...
export const base64UrlEncode = (data: string): string =>
	pipe(btoa(data), stripPadding, toUrlSafe);
